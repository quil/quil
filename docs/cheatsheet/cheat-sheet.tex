...
\colouredbox{blue}{
  ...
  \begin{tabularx}{\hsize}{lX}
   ...
   Trig Constants & \cmd{PI HALF-PI THIRD-PI QUARTER-PI TWO-PI DEG-TO-RAD RAD-TO-DEG } \\
  \end{tabularx}
}
\colouredbox{yellow}{
  \section{Output}
...
\colouredbox{pink}{
  ...
  \begin{tabularx}{\hsize}{lX}
   ...
   Utility Macros & \cmd{with-rotation  with-translation  with-scale } \\
  \end{tabularx}
}
\colouredbox{yellow}{
  \section{Typography}
...
\colouredbox{blue2}{
  \section{Sketch}
  \begin{tabularx}{\hsize}{lX}
 Creation & \cmd{sketch defsketch} \\
 Control & \cmd{sketch-stop sketch-start sketch-close} \\
 State   & \cmd{sketch-state} \\
 Config Keywords & \cmd{:title :size :renderer} \\
...
\colouredbox{grey}{
  \section{Reflection}
  \begin{tabularx}{\hsize}{lX}
API Exploration & \cmd{show-cats show-fns show-meths} \\
Version & \cmd{processing-version} \\
  \end{tabularx}
}

\colouredbox{orange}{
  \section{Helper Namespaces (not included by default)}
  \begin{tabularx}{\hsize}{lX}
quil.helpers.calc & \cmd{mul-add mode-range} \\
quil.helpers.drawing & \cmd{line-join-points} \\
quil.helpers.seqs & \cmd{range-incl indexed-range-incl indexed-range steps cycle-between tap seq->stream tally perlin-noise-seq} \\
  \end{tabularx}
}
 ...
 \begin{flushright}
  ...
  \verb!$Revision: 1.0.0.0, $Date: 7th April, 2012!\\
